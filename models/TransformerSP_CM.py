...
class TRANSFORMER_SP_CM(torch.nn.Module):

    def __init__(self, args):
        action_space = args.action_space
        hidden_state_sz = args.hidden_state_sz
        super(TRANSFORMER_SP_CM, self).__init__()
        ...
        self.TFencoder = TransformerEncoder(200,512,512,512,512,[101,512],512,1024,4,4,0.2,use_bias=True)
        ...
        self.midmapping = nn.Linear(512, 5)
        self.final_mapping = nn.Linear(10, 1)
    ...
    def new_gcn_embed(self, objstate, class_onehot):
        ...
        x = F.relu(self.midmapping(x)) # (101,5)
        x = torch.cat((x, objstate), dim=1) # (101,5) -> (101,10)  
        ...
        x = self.final_mapping_t(x) # (1,512)
